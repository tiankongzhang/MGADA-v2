...
class Sim10kDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_dir, split, use_difficult=False, transforms=None, is_sample=False, domain='source'):
        self.root = data_dir
    ...
    def __getitem__(self, index):
        ...
        metas = {'img_mask':fimg_mask, 'domain':self.domain}
        if self.transforms is not None:
            img, target, metas = self.transforms(img, target, metas)

        return img, target, index, metas
