...
def evaluate_box_proposals(
    ...
    if thresholds is None:
        ...
        thresholds = torch.arange(0.5, 0.51 + 1e-5, step, dtype=torch.float32)
    recalls = torch.zeros_like(thresholds)
    precison = torch.zeros_like(thresholds)
    # compute recall for each iou threshold
    for i, t in enumerate(thresholds):
        recalls[i] = (gt_overlaps >= t).float().sum() / float(num_pos)
        precison[i] = (gt_overlaps >= t).float().sum() / float(num_pred)
...
def compute_thresholds_for_classes(coco_eval):
    ...
    scores = scores[0, :, :, 0, -1]
    recall_ = coco_eval.eval['recall'][0, :, 0, -1]
