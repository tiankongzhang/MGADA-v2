...
class ADCompose(object):
    ...
    def __call__(self, imgs, mask=None):
        ...
        return imgs, params, mask

class ADRandomCrop(object):
...
class ADCenterCrop(object):
    ...
    def __call__(self, img, mask=None):
        ...
        img = ImageOps.expand(img, border=(ld, td, rd, bd), fill=0)
        if mask is not None:
            mask = ImageOps.expand(mask, border=(ld, td, rd, bd), fill=0)
...
class ADAdjustSaturation(object):
    ...
    def __call__(self, img, mask=None):
        return (tf.adjust_saturation(img,
                                    random.uniform(1 - self.saturation,
                                                   1 + self.saturation)), None, mask)


class ADAdjustHue(object):
...
class ADAdjustBrightness(object):
...
class ADRandomHorizontallyFlip(object):
    def __init__(self, p):
        self.p = p
    ...
    def __call__(self, img, mask=None):
        ...
        return img, params, mask


class ADRandomVerticallyFlip(object):
    def __init__(self, p):
        self.p = p
...
'''
class ADRandomTranslate(object):
    ...
    def __call__(self, img, mask=None):
        ...
        x_offset = int(1 * (random.random() - 0.5) * self.offset[0])
        y_offset = int(1 * (random.random() - 0.5) * self.offset[1])
        
        params['Translate'] = abs(x_offset)
        ...
        return tf.pad(cropped_img,
                     padding_tuple,
                     padding_mode='reflect'), params, mask


class ADRandomRotate(object):
    ...
    def __call__(self, img, mask=None):
        ...
        return (
            tf.affine(img,
                      ...
                      fillcolor=(0, 0, 0),
                      shear=0.0), params, mask)


class ADRandomSizedCrop(object):
    def __init__(self, size):
        self.size = size
...
class ADRandomSized(object):
    def __init__(self, size, scale_sz=[0.5, 1.1]):
        self.size = size
        self.crop = ADRandomCrop(self.size)
...
class ADIaRandGaussianNoise_v0:
    ...
    def __call__(self, image, mask=None):
        ...
        return image, params, mask

class ADIaRandGaussianNoise_v1:
    # Note: there is significant difference
    # with OpenCV implementation how sigma is
    # computed for the given radius
