...
class DetectronCheckpointer(Checkpointer):
    ...
    def save(self, name, **kwargs):
        ...
        '''
        data['model_docker'] = self.model.state_dict()

        if self.optimizer is not None:
            for key, value in self.optimizer.items():
               data["optimizer_"+key] = value.state_dict()
                   

        if self.scheduler is not None:
            for key, value in self.scheduler.items():
                data["scheduler_"+key] = value.state_dict()
    ...
    def load(self, f=None, load_dis=True, load_opt_sch=False):
        ...
        if load_opt_sch:
            if "optimizer_" in checkpoint and self.optimizer:
                ...
                for key, value in self.optimizer.items():
                     lkey = "optimizer_" + key
                     if lkey in checkpoint.keys():
                         self.optimizer[key].load_state_dict(checkpoint.pop(lkey))
            ...
            if "scheduler_" in checkpoint and self.scheduler:
                ...
                for key, value in self.scheduler.items():
                    lkey = "scheduler_" + key
                    if lkey in checkpoint.keys():
                        self.scheduler[key].load_state_dict(checkpoint.pop(lkey))
    ...
    def _load_model(self, checkpoint, load_dis=True):
        ...
        '''
        #print('---oo:', checkpoint['model'].keys(), checkpoint['model']['model_docker'].keys())
        
        #print('---oo:', checkpoint.keys(), checkpoint['model'].keys())# checkpoint['model'].keys()
        if 'model' in checkpoint.keys():
             lcheckpoint = checkpoint['model']
        else:
             lcheckpoint = checkpoint
        if "model_docker" in lcheckpoint.keys():
            # load checkpoint of our model
            #print(lcheckpoint.pop("model_docker").keys())
            #print(self.model)
            load_state_dict(self.model, lcheckpoint.pop("model_docker"))
