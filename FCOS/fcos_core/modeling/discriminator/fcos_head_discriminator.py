...
class FCOSDiscriminator(nn.Module):
    ...
    def forward(self, feature, target, domain='source', ims_mask=None):
        ...
        lims_mask = F.interpolate(ims_mask, size=[feature.size(-2), feature.size(-1)], mode="bilinear")[:,0]
        lims_mask = (lims_mask > 0.5).float()
        target = torch.full(x.shape, target, dtype=torch.float, device=x.device)
    
        loss = self.loss_fn(x, target)
        floss = (loss.mul(lims_mask).sum()).div(lims_mask.sum() + 1e-6)
        return floss
